...
void SetTrigger(uint16_t code, unsigned int codeLength)
{
    ...
    for(i=1; i <= codeLength; i++)
    {
        ...
        __delay_cycles(5); //pulse width 5 us
    }
}
...
void I2Cslave_init(void)
{
    ...
    UCB0IE     = UCSTTIE + UCSTPIE + UCRXIE0 + UCTXIE0;     // Enable STT, STP, RX0 and TX0 interrupts
}
...
__interrupt void USCI_B0_ISR(void)
{
    switch(__even_in_range(UCB0IV, 0x1e))
    {
        ...
        case USCI_I2C_UCSTTIFG:                             // UCSTTIFG (Start condition)
            tx_index = 0;
            break;
        case USCI_I2C_UCSTPIFG:                             // UCSTPIFG (Stop condition)
            if (state == send_data) {
...
int main(void) {
    ...
    uint32_t signature = lfsr(n, seed);                       // 10-bit pseudo random number output using a 32-bit LFSR
    ...
    while(1) {
        switch (state) {
            case receive_trigger:
                // handled by ISR, should remain in this state until trigger received
                break;
            case transmit_pulse:
            ...
            case receive_pulse:
                ...
                break;
            case send_data:
                // handled by ISR, should remain in this state until data requested
                break;
        }
    }
}
